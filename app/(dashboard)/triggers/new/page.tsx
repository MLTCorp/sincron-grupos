...
type DestinoMensagem = "mesmo_grupo" | "outros_grupos"
type TipoEnvio = "nova_mensagem" | "responder" | "encaminhar"
...
export default function NewTriggerPage() {
          ...
          <>
            ...
            >
              <div className="flex items-center space-x-1.5">
                ...
                <Label htmlFor="and" className="font-normal cursor-pointer text-xs sm:text-sm">
                  Todas (E)
                </Label>
              </div>
              <div className="flex items-center space-x-1.5">
                <RadioGroupItem value="OR" id="or" />
                <Label htmlFor="or" className="font-normal cursor-pointer text-xs sm:text-sm">
                  Qualquer (OU)
                </Label>
              </div>
            </RadioGroup>
              ...
              {formData.regras.map((regra, index) => (
                <div key={index} className="p-2 sm:p-2.5 rounded-lg bg-muted/50 space-y-2 sm:space-y-0 sm:flex sm:gap-1.5 sm:items-center">
                  <div className="flex gap-1.5 flex-1">
                    <Select
                      value={regra.campo}
                      onValueChange={(val) => updateRegra(index, "campo", val)}
                    >
                    ...
                    <Select
                      ...
                      onValueChange={(val) => updateRegra(index, "operador", val)}
                    >
  ...
  const renderStep3 = () => (
    <div className="space-y-4">
      ...
      {/* Configuracao especifica por tipo de acao */}
      <div className="space-y-3 pt-3 border-t">
        ...
        {formData.tipoAcao === "enviar_mensagem" && (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              {/* Seletor de grupos quando "outros_grupos" */}
            ...
            {/* Mensagem (apenas para nova_mensagem e responder) */}
            {(formData.configAcao.tipo_envio as string || "nova_mensagem") !== "encaminhar" && (
              <div className="space-y-1.5">
                ...
                />
                <p className="text-[10px] text-muted-foreground">
                  Use {"{nome}"} para autor, {"{grupo}"} para nome do grupo
                </p>
              </div>
            )}

            {/* Opção de mencionar (apenas para nova mensagem no mesmo grupo) */}
             ...
             (formData.configAcao.destino as string || "mesmo_grupo") === "mesmo_grupo" && (
              <div className="flex items-center space-x-2 p-2 rounded-lg bg-muted/50">
                <Checkbox
                  ...
                  checked={(formData.configAcao.mencionar_autor as boolean) || false}
                  onCheckedChange={(checked) => updateConfigAcao("mencionar_autor", checked)}
                />
                <Label htmlFor="mencionar" className="font-normal cursor-pointer text-xs">
                  Mencionar autor
                </Label>
              </div>
            )}

            {/* Opcoes de encaminhar */}
              ...
              <div className="space-y-2">
                <div className="p-2 rounded-lg bg-muted/50 border-l-2 border-foreground/30">
                  <p className="text-[10px] sm:text-xs text-muted-foreground">
                    Encaminha a mensagem original mantendo conteudo (texto, midia, etc).
                  </p>
                </div>

                <div className="flex items-center space-x-2 p-2 rounded-lg bg-muted/50">
                  <Checkbox
                    id="enviar_introducao"
                    className="h-3.5 w-3.5"
                    checked={(formData.configAcao.enviar_introducao as boolean) || false}
                    onCheckedChange={(checked) => updateConfigAcao("enviar_introducao", checked)}
                  />
                  <Label htmlFor="enviar_introducao" className="font-normal cursor-pointer text-xs">
                    Enviar introducao antes
                  </Label>
                </div>

                {(formData.configAcao.enviar_introducao as boolean) && (
                  <div className="space-y-1.5 pl-4 border-l-2 border-foreground/20">
                    <Label htmlFor="mensagem_introducao" className="text-xs text-muted-foreground">
                      Introducao
                    </Label>
                    <Textarea
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="https://..."
                className="h-8 sm:h-9 text-sm"
                value={(formData.configAcao.url as string) || ""}
                onChange={(e) => updateConfigAcao("url", e.target.value)}
              />
            </div>
            <div className="flex items-center space-x-2 p-2 rounded-lg bg-muted/50">
              <Checkbox
                id="incluir_mensagem"
                className="h-3.5 w-3.5"
                checked={(formData.configAcao.incluir_mensagem as boolean) !== false}
                onCheckedChange={(checked) => updateConfigAcao("incluir_mensagem", checked)}
              />
              <Label htmlFor="incluir_mensagem" className="font-normal cursor-pointer text-xs">
                Incluir dados da mensagem
              </Label>
            </div>
          </div>
        )}
        ...
        {formData.tipoAcao === "notificar_admin" && (
          <div className="space-y-1.5">
            ...
            <Textarea
              id="notificacao"
              placeholder="Alerta: foi detectado..."
              value={(formData.configAcao.mensagem as string) || ""}
              onChange={(e) => updateConfigAcao("mensagem", e.target.value)}
              className="min-h-[70px] text-sm"
            />
        ...
        {formData.tipoAcao === "acionar_bot" && (
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              {agentes.length > 0 ? (
                <Select
                  value={(formData.configAcao.id_agente as number)?.toString() || ""}
                  onValueChange={(val) => updateConfigAcao("id_agente", Number(val))}
                >
                  <SelectTrigger className="h-8 sm:h-9 text-sm">
                  ...
                  <SelectContent>
                    {agentes.map(agente => (
                      <SelectItem key={agente.id} value={agente.id.toString()} className="text-sm">
                        {agente.nome}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            {agentes.length > 0 && (
              <div className="flex items-center justify-between">
                ...
                <Link href="/ai">
                  <Button variant="outline" size="sm" className="h-6 text-[10px] sm:h-7 sm:text-xs">
                    Gerenciar
                  </Button>
                </Link>
              </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 p-2 rounded-lg bg-muted/50">
              <Checkbox
                id="notificar_autor"
                className="h-3.5 w-3.5"
                checked={(formData.configAcao.notificar_autor as boolean) || false}
                onCheckedChange={(checked) => updateConfigAcao("notificar_autor", checked)}
              />
              <Label htmlFor="notificar_autor" className="font-normal cursor-pointer text-xs">
                Avisar autor sobre remocao
              </Label>
            </div>
            {Boolean(formData.configAcao.notificar_autor) && (
              <div className="space-y-1.5 pl-4 border-l-2 border-foreground/20">
                ...
                <Input
                  id="aviso"
                  placeholder="Mensagem removida por violar regras"
                  className="h-8 sm:h-9 text-sm"
                  value={(formData.configAcao.mensagem_aviso as string) || ""}
                  onChange={(e) => updateConfigAcao("mensagem_aviso", e.target.value)}
                />
              </div>
            )}
          </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-2 grid-cols-2">
          <div className="p-2.5 sm:p-3 rounded-lg bg-muted/50">
            ...
            <div className="flex items-center gap-1.5">
              {escopoCor && (
                <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: escopoCor }} />
              )}
          ...
          <div className="p-2.5 sm:p-3 rounded-lg bg-muted/50">
            <p className="text-[10px] sm:text-xs text-muted-foreground mb-0.5">Condicoes</p>
            <p className="font-medium text-xs sm:text-sm">
              {formData.regras.length > 0 ? `${formData.regras.length} condicao(oes)` : "Nenhuma"}
            </p>
          </div>
        </div>
        ...
        {formData.tipoAcao === "enviar_mensagem" && (
          <div className="p-2.5 sm:p-3 rounded-lg bg-muted/50 space-y-2">
            ...
            {(formData.configAcao.tipo_envio as string) === "encaminhar" && (formData.configAcao.enviar_introducao as boolean) && (
              <div className="pt-2 border-t border-border/50">
                <p className="text-[10px] text-muted-foreground mb-1">Introducao:</p>
                <p className="text-xs bg-background/50 p-1.5 rounded line-clamp-2">{(formData.configAcao.mensagem_introducao as string) || "(vazia)"}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center flex-1">
              ...
              {i < STEPS.length - 1 && (
                <div className={cn(
                  "flex-1 h-0.5 sm:h-1 mx-1.5 sm:mx-2 rounded-full transition-all",
          ...
          >
            {loading ? (
              ...
              <>
                ...
                <span className="hidden sm:inline">Criar Gatilho</span>
